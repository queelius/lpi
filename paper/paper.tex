...
\State Initialize: $n \gets 0$ \Comment{Total observations counter}
\For{$t = 1$ to $T$} \Comment{Iterate over batches}
...
\State $n \gets 0$ \Comment{Total observations counter}
\For{batch $B$ in data stream} \Comment{Process batches sequentially}
\State // Adaptive structure selection (every $k$ batches)
\If{batch index $\mod k = 0$}
\State $\text{struct} \gets \mathcal{S}(\mathcal{I}_n, B)$ \Comment{Choose: diagonal, block, Kronecker, etc.}
\EndIf
\State // Information accumulation from batch
\State $s_B \gets \nabla_\theta \ell_{B}(\theta_n)$ \Comment{Batch gradient}
...
\State $n \gets n + |B|$ \Comment{Update total observation count}
\EndFor
\end{algorithmic}
\end{algorithm}
...
\bibitem{amari1998natural} Amari, S. (1998). Natural gradient works efficiently in learning. Neural Computation, 10(2), 251--276.
\bibitem{amari2016information} Amari, S. (2016). Information Geometry and Its Applications. Springer.
\bibitem{devlin2018bert} Devlin, J., Chang, M.-W., Lee, K., Toutanova, K. (2018). BERT: Pre-training of deep bidirectional transformers for language understanding. arXiv:1810.04805.
\bibitem{efron1978assessing} Efron, B., Hinkley, D. V. (1978). Assessing the accuracy of the maximum likelihood estimator: Observed versus expected Fisher information. Biometrika, 65(3), 457--487.
\bibitem{fisher1925statistical} Fisher, R. A. (1925). Statistical Methods for Research Workers. Oliver and Boyd.
\bibitem{guo2017calibration} Guo, C., Pleiss, G., Sun, Y., Weinberger, K. Q. (2017). On calibration of modern neural networks. ICML (PMLR 70), 1321--1330.
\bibitem{hochreiter1997flat} Hochreiter, S., Schmidhuber, J. (1997). Flat minima. Neural Computation, 9(1), 1--42.
\bibitem{jeffreys1939theory} Jeffreys, H. (1939). Theory of Probability. Oxford University Press.
\bibitem{keskar2017large} Keskar, N. S., Mudigere, D., Nocedal, J., Smelyanskiy, M., Tang, P. T. P. (2017). On large-batch training for deep learning: Generalization gap and sharp minima. ICLR.
\bibitem{kingma2014adam} Kingma, D. P., Ba, J. (2014). Adam: A method for stochastic optimization. arXiv:1412.6980.
\bibitem{kirkpatrick2017overcoming} Kirkpatrick, J. et al. (2017). Overcoming catastrophic forgetting in neural networks. PNAS, 114(13), 3521--3526.
\bibitem{ljung1983theory} Ljung, L. (1983). Theory and Practice of Recursive Identification. MIT Press.
\bibitem{martens2015optimizing} Martens, J., Grosse, R. (2015). Optimizing neural networks with Kronecker-factored approximate curvature. ICML (PMLR 37), 1107--1115.
\bibitem{parisi2019continual} Parisi, G. I., Kemker, R., Part, J. L., Kanan, C., Wermter, S. (2019). Continual lifelong learning with neural networks: A review. Neural Networks, 113, 54--71.
\bibitem{radford2018improving} Radford, A., Narasimhan, K., Salimans, T., Sutskever, I. (2018). Improving language understanding by generative pre-training. OpenAI Blog.
\bibitem{rao1945information} Rao, C. R. (1945). Information and the accuracy attainable in the estimation of statistical parameters. Bull. Calcutta Math. Soc., 37, 81--91.
\bibitem{robert2007bayesian} Robert, C. P. (2007). The Bayesian Choice. Springer.
\bibitem{settles2009active} Settles, B. (2009). Active learning literature survey. Technical Report 1648, University of Wisconsin--Madison.
\bibitem{tierney1986accurate} Tierney, L., Kadane, J. B. (1986). Accurate approximations for posterior moments and marginal densities. JASA, 81(393), 82--86.
\bibitem{vanderVaart1998asymptotic} van der Vaart, A. W. (1998). Asymptotic Statistics. Cambridge University Press.
\bibitem{holtzman2019curious} Holtzman, A., Buys, J., Du, L., Forbes, M., Choi, Y. (2020). The curious case of neural text degeneration. ICLR.
\end{thebibliography}
